...
pub async fn scan_file(State(state): State<AppState>) -> impl IntoResponse {
    let storage_roots = entity::storage_root::Entity::find()
        .all(&state.db)
        .await
        .unwrap();
